...
// ========================================================================
// Time:   13min
// Submit: 2 Line 67 didn't think clearly whether it should be larger of
//  smaller
// ========================================================================
...
var wiggleMaxLength = function(nums) {
  ...
  let count = 2;
  let checkSmaller = nums[i - 1] > nums[i] ? false : true;
  nums[0] = nums[i - 1];
  nums[1] = nums[i];

  for (; i < nums.length - 1; i++) {
    if (checkSmaller && nums[i] > nums[i + 1]) {
      nums[count] = nums[i + 1];
      count++;
      checkSmaller = !checkSmaller;
    } else if (!checkSmaller && nums[i] < nums[i + 1]) {
      nums[count] = nums[i + 1];
      count++;
      checkSmaller = !checkSmaller;
    }
